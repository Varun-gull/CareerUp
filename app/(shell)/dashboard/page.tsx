  ...
  return (
    <main className="page-shell space-y-5">
      ...
      {searchParams?.message && (
        <p className="rounded-2xl border border-[#B8A99A]/35 bg-[#FFFDF9] p-3 text-sm font-bold text-[#7A5438]">{searchParams.message}</p>
      )}

      <section className="dashboard-overlap space-y-5">
        <div className="dashboard-layer">
          <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
            <StatCard icon={Sparkles} label="Total XP" value={profile.xp.toLocaleString()} tone="bg-[#7A5438] text-white" />
            <StatCard icon={Flame} label="Day streak" value={profile.streak.toLocaleString()} tone="bg-[#C7A47B] text-white" />
            <StatCard icon={CheckCircle2} label="Applications sent" value={appliedCount.toLocaleString()} tone="bg-[#E2D2BF] text-[#7A5438]" />
            <StatCard icon={Trophy} label="Offers" value={offerCount.toLocaleString()} tone="bg-[#FFFDF9] text-[#7A5438] ring-1 ring-[#B8A99A]/45" />
          </div>
        </div>
        ...
        <div className="dashboard-layer grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px] xl:items-stretch">
          ...
          <section className="card flex h-full flex-col p-5">
            <div className="flex items-end justify-between gap-3">
              ...
              <span className="rounded-full bg-[#E2D2BF] px-3 py-1 text-xs font-bold text-[#7A5438] ring-1 ring-[#B8A99A]/35">3 today</span>
            </div>
