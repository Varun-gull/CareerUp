  ...
  return (
    <>
      ...
      <main className="page-shell">
        ...
        {inviteId && (
          <form action={sendFriendRequestById} className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[#b0cdd9] bg-[#C8D9E6]/40 p-4">
            <div>
              <p className="font-black text-[#2F4156]">Friend invite opened</p>
              <p className="text-sm font-bold text-[#2F4156]">Send a request to add this CareerUp profile.</p>
            </div>
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-[1fr_360px]">
          <form action={sendFriendRequest} className="card grid gap-4 p-5">
            <div className="flex items-center gap-3">
              <span className="flex h-11 w-11 items-center justify-center rounded-lg bg-[#C8D9E6]/40 text-[#2F4156]">
                <UserPlus size={20} />
              </span>
              <div>
            ...
            <label className="grid gap-2 text-sm font-bold text-slate-700">
              Email
              <input name="email" type="email" className="rounded-lg border border-slate-200 px-4 py-3 outline-none focus:border-[#567C8D]" placeholder="friend@example.com" required />
            </label>
          ...
          {friends.map((friend) => (
            <article key={friend.id} className="rounded-lg border border-slate-200 bg-white p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <UsersRound size={18} className="text-[#567C8D]" />
              </div>
              <div className="mt-4 flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-black text-[#2F4156]">{friend.xp.toLocaleString()} XP</p>
                ...
                <div className="flex gap-2">
                  {action === "accept" && (
                    <form action={acceptFriendRequest}>
                      ...
                      <button className="inline-flex min-h-10 items-center rounded-lg bg-[#567C8D] px-3 text-sm font-bold text-white" aria-label={`Accept ${friend.name}`}>
                        <Check size={16} />
                      </button>
                    </form>
                  )}
