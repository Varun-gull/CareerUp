  ...
  return (
    <>
      ...
      <main className="page-shell">
        ...
        {inviteId && (
          <form action={sendFriendRequestById} className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-lavender bg-lavender/50 p-4">
            <div>
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-[1fr_360px]">
          <form action={sendFriendRequest} className="card grid gap-4 p-5">
            <div className="flex items-center gap-3">
              <span className="flex h-11 w-11 items-center justify-center rounded-lg bg-lavender/50 text-brand">
                <UserPlus size={20} />
              </span>
              <div>
            ...
            <label className="grid gap-2 text-sm font-bold text-slate-700">
              Email
              <input name="email" type="email" className="rounded-lg border border-slate-200 px-4 py-3 outline-none focus:border-brand" placeholder="friend@example.com" required />
            </label>
                ...
                <div className="flex gap-2">
                  {action === "accept" && (
                    <form action={acceptFriendRequest}>
                      ...
                      <button className="inline-flex min-h-10 items-center rounded-lg bg-brand px-3 text-sm font-bold text-white" aria-label={`Accept ${friend.name}`}>
                        <Check size={16} />
                      </button>
                    </form>
                  )}
