            ...
            <section className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)]">
              <form action={saveInterviewAnswer} className="card min-w-0 p-5">
                <div className="mb-5 flex items-start gap-3">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-[#C8D9E6]/40 text-[#2F4156]">
                ...
                <label className="grid min-w-0 gap-2 text-sm font-bold text-slate-700">
                  Prompt
                  <select name="prompt" className="w-full min-w-0 max-w-full truncate rounded-lg border border-slate-200 bg-white px-4 py-3 outline-none focus:border-[#567C8D]">
                ...
                <label className="mt-4 grid gap-2 text-sm font-bold text-slate-700">
                  Notes
                  <textarea name="notes" rows={3} className="resize-none rounded-lg border border-slate-200 px-4 py-3 outline-none focus:border-[#567C8D]" placeholder="Keywords, company tie-in, or follow-up examples." />
                </label>
              ...
              <aside className="card p-5">
                <p className="text-sm font-black text-[#567C8D]">Answer formula</p>
                  ...
                  {answers.map((answer) => (
                    <article key={answer.id} className="card p-5">
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        <div>
                          <p className="text-sm font-black text-[#567C8D]">{answer.createdAt}</p>
                      ...
                      {answer.notes && <p className="mt-4 rounded-lg bg-[#C8D9E6]/40 p-3 text-sm leading-6 text-slate-700">{answer.notes}</p>}
                    </article>
                  ))}
                </div>
              ) : (
...
function StarTextarea({ name, label, placeholder }: { name: string; label: string; placeholder: string }) {
  return (
    <label className="grid gap-2 text-sm font-bold text-slate-700">
      {label}
      <textarea name={name} rows={5} className="resize-none rounded-lg border border-slate-200 px-4 py-3 outline-none focus:border-[#567C8D]" placeholder={placeholder} />
    </label>
  );
}
