  ...
  return (
    <article className={compact ? "rounded-lg border border-slate-200 bg-white p-4 shadow-sm" : "card p-5"}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-sm font-bold text-brand">{application.company}</p>
      ...
      <div className={compact ? "mt-4 grid gap-3 text-sm text-slate-600" : "mt-4 grid gap-3 text-sm text-slate-600 sm:grid-cols-2"}>
        ...
        {sourceIsUrl ? (
          <a className="inline-flex items-center gap-2 font-bold text-brand hover:text-blue-900" href={application.source} target="_blank" rel="noreferrer">
