...
const TIER_COLORS = [
  "",
  "text-[#1B3C53] bg-[#E1EFEB] ring-[#5E7681]/30",
  ...
  return (
    <article className={clsx("card p-5 transition hover:-translate-y-1 hover:shadow-strong", challenge.completed && "ring-2 ring-emerald-400/60")}>
      <div className="flex items-start justify-between gap-4">
        <div className={clsx("flex h-11 w-11 items-center justify-center rounded-2xl shadow-sm", challenge.completed ? "bg-emerald-500" : "bg-[#1B3C53] text-white")}>
          {challenge.completed
        ...
        <div className="flex items-center gap-2">
          ...
          {challenge.completed ? (
            ...
            <span className="rounded-full bg-[#E1EFEB] px-3 py-1 text-xs font-bold text-[#1B3C53] ring-1 ring-[#5E7681]/30">+{challenge.xp} XP</span>
          )}
        </div>
      </div>
