...
export function LeaderboardTable({ users, currentUserId, emptyMode = "global" }: { users: LeaderboardUser[]; currentUserId?: string; emptyMode?: "global" | "friends" }) {
  ...
  return (
    <div className="card overflow-hidden">
      <div className="grid grid-cols-[56px_1fr_100px] gap-4 border-b border-slate-800 bg-slate-950 px-5 py-4 text-xs font-black uppercase text-slate-300 sm:grid-cols-[56px_1fr_150px_110px]">
        <span>#</span>
        <span>Player</span>
        <span>XP</span>
      ...
      {sorted.map((user, index) => (
        <div
          key={user.id}
          className="grid grid-cols-[56px_1fr_100px] gap-4 border-b border-slate-800 px-5 py-4 transition last:border-0 hover:bg-sky/10/50 sm:grid-cols-[56px_1fr_150px_110px]"
        >
          ...
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-slate-700 bg-white">
              {user.schoolLogoUrl ? (
          ...
          <span className="hidden items-center gap-1 font-bold text-slate-400 sm:inline-flex">
