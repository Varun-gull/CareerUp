...
export function ProfileDropdown({ initials, loggedIn }: { initials: string; loggedIn: boolean }) {
  ...
  if (!loggedIn) {
    return (
      <Link
        href="/login"
        className="flex h-10 min-w-10 items-center justify-center rounded-lg border border-slate-200 px-2 text-sm font-black text-slate-700 transition hover:border-accent hover:text-brand"
      >
        <UserRound size={20} />
      </Link>
    );
      ...
      {open && (
        <div className="absolute right-0 top-12 z-50 w-44 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg">
          <Link
            ...
            className="flex items-center gap-3 px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 hover:text-brand"
          >
