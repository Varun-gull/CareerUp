        ...
        {groupedApplications.map((column) => (
          <div key={column.status} className="min-w-0 rounded-3xl border border-white/70 bg-white/85 p-3 shadow-sm backdrop-blur">
            ...
            {column.applications.length > 0 ? (
                ...
                {column.applications.map((application) => {
                  ...
                  return (
                    <article key={application.id} className="min-w-0 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                      ...
                      <div className="mt-3 grid gap-2 border-t border-slate-200 pt-3 text-xs font-bold text-slate-500">
              ...
              <div className="rounded-2xl border border-dashed border-slate-200 bg-white/85 p-4 text-sm font-bold text-slate-500">No roles here.</div>
            )}
          </div>
        ))}
      </div>
